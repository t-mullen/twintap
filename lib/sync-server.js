  ...
  ws.on('connection', (socket) => {
    ...
    socket.on('__pairID__', (pairID) => {
      ...
      if (p.sockets.length === 2) {
        ...
        // forward all events
        p.sockets.forEach((socket, i) => {
          const other = p.sockets[(i + 1) % 2]
          socket.on('__barrier__', (data) => { other.sendEvent('__barrier__', data) })
          socket.on('__send__', (data) => { other.sendEvent('__send__', data) })
        })
      } else {
        // queue events until other socket connectsa
        socket.on('__barrier__', (data) => {
          if (p.sockets.length === 2) return

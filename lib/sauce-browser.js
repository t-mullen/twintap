...
function SauceBrowser (conf, opt) {
  ...
  this._opt = opt
  this.debug = debug('twintap:sauce:' + conf.browser + ':' + conf.version)
...
SauceBrowser.prototype._start = function (pairID, instanceID) {
  var self = this
  var conf = self._conf
  ...
  self.controller = createTestServer(self._opt, function (err, rootUrl) {
    if (err) {
      return self.shutdown(err)
    }
    const url = rootUrl + '?pairID=' + pairID + '&instanceID=' + instanceID
    ...
    // Workaround for https://github.com/twintap/browsers/issues/3
